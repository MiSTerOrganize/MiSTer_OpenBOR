...
# Sweep any rogue OpenBOR_4086 binary left over from a previous
# daemon instance that failed to kill its child cleanly. We use
# /proc inspection so we only kill OUR core's binary, not the
# sister core (OpenBOR_7533). Both cores' binaries are named
...
while true; do
    ...
    if [ -n "$CHILD" ]; then
        ...
        if [ "$CUR" != "OpenBOR_4086" ]; then
            ...
            # (OpenBOR_7533) tries to render. Both binaries map the
